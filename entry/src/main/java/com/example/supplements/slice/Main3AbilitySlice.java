...
import com.example.supplements.Utils.GsonUtils;
import com.example.supplements.Utils.LogUtils;
import com.example.supplements.Utils.MapUtils;
import com.example.supplements.Utils.PermissionsUtils;
import com.example.supplements.beans.InputTipsResult;
import com.example.supplements.map.MapDataHelper;
import com.example.supplements.map.MapManager;
import com.example.supplements.provider.InputTipsProvider;
 ...
 * @since 2021-03-12
 */
public class Main3AbilitySlice extends AbilitySlice
        implements Component.ClickedListener, IAbilityContinuation, MapDataHelper.DataCallBack {
    private static final String TAG = Main3AbilitySlice.class.getSimpleName();
    ...
    public void onStart(Intent intent) {
        ...
        super.setUIContent(ResourceTable.Layout_ability3_main);
        findComponentById();
        initView();
            ...
            case ResourceTable.Id_nav_translate:
                if (tinyMap.getMapElements() != null && !tinyMap.getMapElements().isEmpty()) {
                    ...
                    if (elementStr.length() >= 200 * 1024 / 2) {
                        MapUtils.showToast(Main3AbilitySlice.this, "您输入的距离太远，请重新输入");
                        return;
                    }
                }
